...
main (int argc, const char* argv[]) 
{
    ...
    for (auto fn : sol_fn) {
        ...
        sols.push_back (sol);
        cout << "  " << sol->reaction_count() << " reactions used" << endl;
...
//          "123 2345678901 234567890"
    ...
    for (size_t k = 0; k < scenario.num_reactions(); k++) {
        if (count[k] > 0) {
            sorter.add (k, -scenario.reaction(k)->obj_coeff());
        }
    }
    sorter.doSort();
    double x = 0.0f;
    for (size_t ik = 0; ik < sorter.size(); ik++) {
        size_t k = sorter[ik];
        dig.xTic (x, scenario.reaction(k)->name());
        x += sols.size();
    }
    for (size_t j = 0; j < sols.size(); j++) {
        ...
        for (size_t ik = 0; ik < sorter.size(); ik++) {
            size_t k = sorter[ik];
            double flux = sols[j]->flux(k);
            auto react = scenario.reaction(k);
            if (flux > 0.0f) 
                dig.draw (x, 0.0f, x, react->obj_coeff());
            x += sols.size();
        }
    }
    dig.waitAndWipe();
